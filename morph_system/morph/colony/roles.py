...
def builder(adapter: AgentAdapter, cwd: Path, task: str, hypothesis: str, stress: list[tuple[str, float]], antibodies: list[dict]) -> str:
    ...
    return adapter.build(cwd, prompt)


def predator(adapter: AgentAdapter, cwd: Path, base: str, task: str) -> Review:
    diff = _diff(cwd, base)
