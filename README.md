...
> 16. The hero was a static image of three strands converging, boxed in a card. It is now the header's **ground**: a full-bleed canvas carrying a particle field advected along three helical strands whose radius collapses into a single point, perturbed by layered turbulence, brightening as it accelerates inward, with trails accumulated by compositing over a translucent clear rather than wiping the frame. Real perspective projection, slow camera yaw, additive blending, no library. Dropping the inlined WebP took the page from **330 KB to 230 KB**; the asset now serves as the `og:image` the page previously lacked. `prefers-reduced-motion` gets a settled still frame painted once and held.
> 17. The lifecycle diagram needed 760px for its three side-by-side lanes, so on a phone it forced **horizontal scrolling inside a 720px-tall figure**. Added a compact variant below 820px that collapses the three lanes into three coloured tracks plus one shared phase row, and folds "6 states scored" into the winner and apoptosis into the exit: **317×427px at 390px wide, no horizontal scroll**, down from 760×720 plus panning. Both variants are now animated with CSS — flow travelling the connectors, each clone stage lighting in turn, the winner breathing — and an explicit `prefers-reduced-motion` rule restores the plain diagram, since the page-wide rule freezes animations on their *last* keyframe and would otherwise leave every phase box dimmed.
>
>     Measured, not assumed: masking the full-bleed canvas with CSS `mask-image` cost **36fps vs 60fps**, because it forces a compositing pass over the whole canvas every frame. The readability falloff is now evaluated per drawn particle inside the render loop instead — same look, **59/57/61fps** at 1440/1280/390px. The canvas ground is `var(--void)`, the same colour as the page, so its edges stay invisible without a mask.
>
